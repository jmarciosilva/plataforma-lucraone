<x-layouts.app title="produtos" :tenantNome="$tenantNome" :breadcrumbs="$breadcrumbs">
    ...
    <x-card class="mb-6">
        <form method="GET" action="{{ route('catalog.products.index') }}" class="grid grid-cols-1 gap-4 lg:grid-cols-6">
            <x-form-group nome="search" rotulo="busca" class="lg:col-span-2">
                <x-input nome="search" :valor="request('search')" placeholder="sku ou nome" />
            </x-form-group>
    ...
    <x-table :cabecalhos="['produto', 'empresa', 'status', 'categorias', 'preços', '']" :paginacao="$products">
        ...
        @empty
            <tr>
                <td colspan="6" class="px-5 py-10 text-center text-sm text-aco">nenhum produto encontrado.</td>
            </tr>
        @endforelse
    </x-table>
</x-layouts.app>
